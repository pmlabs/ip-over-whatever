...
# - a sound chipset that supports 44100 Hz mode (which should be any modern
#   soundcard / sound chip)
# - line out (speaker out) audio port  \ laptops might have a mixed
# - line in audio port                 / "out+mic" ports, but no "in" ports.
# - some audio cables
#
...
#       even though some symbols seem to be flowing in.
...
#
# 2. Q: How do I know the names of Pulse Audio devices?
#    A: pactl list short sinks ← run this to get a list of "line outs"
...
#
# 5. Q: What's the low-level protocol?
...
#       being transmitted".
#
# 6. Q: What's the high-level protocol?
#    A: If the symbol is all 0, that's silence.
#       The top-most and bottom-most bits (i.e. bits 9 and 0) are called
#       "control bits". The rest of the bits (1-8) are "data bits".
...
#       but require to be able to receive only one.
...
#       size symbols, and without the end symbol.
...
#       1-0 and 0-1 (starting with 1-0 for byte 0 of data).
...
#
# 7. Q: Show me a command line that you used for testing.
#    A: python audio.py \
#              -m both \
